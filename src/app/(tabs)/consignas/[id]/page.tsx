...
export default function ConsignaEscribirPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (data?.consigna_id === id) {
        ...
        initialBodyRef.current = b;
      }
    })();
  }, [textId, id]);

  // Ajustar altura del textarea del título
  useEffect(() => {
  ...
  async function handleGuardar() {
    ...
    const usarConsigna = consigna && !consignaQuitada;
    if (id && !consigna && !consignaQuitada) {
    ...
    const payload = {
      ...
      consigna_id: usarConsigna ? consigna!.id : null,
    };

    if (textId) {
      const { error } = await supabase
        .from("texts")
        .update({ title: payload.title, body: payload.body, status: payload.status, consigna_id: payload.consigna_id })
        .eq("id", textId)
        .eq("user_id", user.id);
      if (error) {
      ...
      if (data?.id) {
        router.replace(`/consignas/${id}?textId=${data.id}`);
  ...
  function handleConfirmarPublicar() {
    ...
    if (textId) {
      ...
      const supabase = createClient();
      const consignaId = consigna && !consignaQuitada ? consigna.id : null;
  ...
  if (id && !consigna) {
    return (
      <div className="flex min-h-screen flex-col bg-neutral-100 p-4">
        <Link href="/consignas" className="text-red text-sm">Volver a consignas</Link>
  ...
  return (
    <div className="flex flex-col min-h-screen bg-neutral-100">
      ...
      <div className="flex-1 min-h-0 flex flex-col overflow-y-auto py-4 pb-8 px-4 max-w-full">
        ...
        {/* Aviso IA: mismo estilo que editar */}
        {showIABox && (
      ...
      {showConfirmarVolver && (
        ...
        >
          <div
            ...
            className="w-full max-w-sm bg-white rounded-2xl p-6 shadow-lg"
          >
            <h2 id="confirmar-volver-title" className="text-lg font-bold text-black leading-5 mb-2">
              ¿Descartar cambios?
            </h2>
            <p className="text-neutral-600 text-sm leading-5 mb-6">
