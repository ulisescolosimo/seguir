...
export default function ConsignaEscribirPage() {
  ...
  useEffect(() => {
    ...
    fetchConsignaById(id)
      .then(setConsigna)
      .finally(() => setLoadingConsigna(false));
  }, [id]);

  // Cargar borrador si hay textId (mismo usuario, misma consigna)
  useEffect(() => {
    if (!textId || !id) return;
  ...
  async function handleGuardar() {
    ...
    if (!consigna) {
      setSaveError("No se encontró la consigna.");
      setSaving(false);
      return;
    }
    ...
    if (textId) {
      const { error } = await supabase
        .from("texts")
        .update({ title: payload.title, body: payload.body, status: payload.status })
        .eq("id", textId)
        .eq("user_id", user.id);
      if (error) {
      ...
      if (data?.id) {
        router.replace(`/consignas/${consigna.id}?textId=${data.id}`);
      }
    }
    setSaving(false);
    router.back();
  ...
  function handleConfirmarPublicar() {
    ...
    if (textId) {
      ...
      // Guardar primero para obtener textId y luego ir a publicar
      const supabase = createClient();
      supabase.auth.getUser().then(({ data: { user } }) => {
        if (!user || !consigna) return;
        supabase
          .from("texts")
          .insert({
            user_id: user.id,
  ...
  if (id && loadingConsigna) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-neutral-100">
        <p className="text-neutral-500 text-sm">Cargando consigna...</p>
      </div>
    );
  }
  if (id && !consigna) {
    return (
      <div className="flex min-h-screen flex-col bg-neutral-100 p-4">
        <Link href="/consignas" className="text-orange-700 text-sm">Volver a consignas</Link>
        <p className="mt-4 text-neutral-500 text-sm">Consigna no encontrada.</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex min-h-screen flex-col bg-neutral-100">
      ...
      <main className="flex-1 min-h-0 flex flex-col overflow-y-auto px-5 py-4 pb-12">
        {/* Título del texto (editable) */}
        <div className="shrink-0">
          <input
            type="text"
            value={title}
            onChange={(e) => setTitle(e.target.value)}
            placeholder="Título de tu texto"
            className="w-full text-black text-xl font-bold font-['Inter'] leading-6 bg-transparent border-0 border-b border-transparent focus:outline-none focus:border-neutral-300 placeholder:text-neutral-400 pb-1"
          />
        </div>

        {/* Card consigna: tema o recurso + título consigna + descripción */}
        <div className="mt-4 w-full rounded-2xl shadow-[0px_8px_8px_0px_rgba(0,0,0,0.07)] border border-orange-700 bg-red-50 p-4">
        ...
        {!showIABox && (
          <div className="shrink-0 mt-4 flex justify-end pb-2">
            <button
              ...
              className="w-12 h-12 rounded-2xl bg-orange-700 flex items-center justify-center text-white hover:opacity-90 transition-colors"
        ...
        <div className="shrink-0 pt-6 flex flex-nowrap gap-3 w-full">
          ...
          <button
            ...
            className="flex-1 h-14 min-w-0 rounded-[47px] border-2 border-orange-700 text-orange-700 text-base font-bold leading-5 tracking-wider flex items-center justify-center hover:bg-orange-700/10 transition-colors"
          >
            PUBLICAR
          </button>
        </div>
